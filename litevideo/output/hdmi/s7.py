...
class S7HDMIOutEncoderSerializer(Module):
    def __init__(self, pad_p, pad_n, bypass_encoder=False):
        if not bypass_encoder:
            self.submodules.encoder = ClockDomainsRenamer("hdmi_in0_pix_o")(Encoder())
        ...
        ce = Signal()
        self.sync.hdmi_in0_pix_o += ce.eq(~ResetSignal("hdmi_in0_pix_o"))
        ...
        self.specials += [
            Instance("OSERDESE2",
                ...
                i_OCE=ce,
                i_TCE=0,
                i_RST=ResetSignal("hdmi_in0_pix_o"),
                i_CLK=ClockSignal("hdmi_in0_pix5x_o"), i_CLKDIV=ClockSignal("hdmi_in0_pix_o"),
